...
#### 加载数据
与 `MNIST` 数据集不同，这里需要调用下载的 `cifar10` 和 `cifar10_input` 类来对数据进行下载和处理，得到训练数据和测试数据。
```python
# 载入需要用的库
...
#### 训练
如下代码中的第三行启动前面提到的图片数据增强的线程队列，这里移动使用了 16 个线程来进行加速。注意，如果这里不启动线程，那么后续的 inference 及训练的操作都是无法开启的。
```python
